...
C_SRCS := \
	...
	kernel/drv/blk_ahci.c kernel/drv/virtio_net.c kernel/drv/e1000.c kernel/drv/fb.c kernel/drv/ps2.c \
...
smoke-linux: esp
	@rm -f $(ROOT)/serial.log $(ROOT)/ovmf_vars.fd
	@HEADLESS=1 TIMEOUT_SECS=90 bash $(ROOT)/scripts/run-qemu.sh || true
	@ok=1; 	if grep -a -F -q "HelixBusyBoxOK" $(ROOT)/serial.log 2>/dev/null; then 		echo "SMOKE-LINUX OK (BusyBox)"; 		grep -a -E 'BusyBox|HelixBusy|helixbox|HelixLinux|tmp_write|BB\[' $(ROOT)/serial.log | head -40; 	else 		for pat in "HelixLinuxOK" "helixbox_smoke_done" "tmp_write_ok"; do 			grep -a -F -q "$$pat" $(ROOT)/serial.log 2>/dev/null || { echo "SMOKE-LINUX FAIL $$pat"; ok=0; }; 		done; 		[ "$$ok" = 1 ] && echo "SMOKE-LINUX OK (helixbox)" || { cat $(ROOT)/serial.log; exit 1; }; 	fi; 	if grep -a -F -q "HelixMshOK" $(ROOT)/serial.log 2>/dev/null; then 		echo "SMOKE-LINUX OK (msh pipeline)"; 	fi; 	if grep -a -F -q "HelixCwdOK" $(ROOT)/serial.log 2>/dev/null; then 		echo "SMOKE-LINUX OK (cwd/chdir)"; 	else 		echo "SMOKE-LINUX WARN: HelixCwdOK missing"; 	fi; 	if grep -a -F -q "HelixSigOK" $(ROOT)/serial.log 2>/dev/null; then 		echo "SMOKE-LINUX OK (signals)"; 	else 		echo "SMOKE-LINUX WARN: HelixSigOK missing"; 	fi; 	if grep -a -F -q "BusyBox chain done" $(ROOT)/serial.log 2>/dev/null; then 		echo "SMOKE-LINUX OK (BusyBox 5-applet chain)"; 		grep -a -E 'BB\[' $(ROOT)/serial.log | head -15; 	else 		echo "SMOKE-LINUX WARN: BusyBox chain incomplete"; 	fi
